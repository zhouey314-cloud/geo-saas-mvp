...
# 画像×体裁 精准组合提示词库 v2.0
V2_UGC_TEMPLATES = {
    "P1_G1": """你是一名{行业}领域的普通消费者。请写一篇{word_range}字的焦虑共鸣文。结构严格如下：\n1. [痛点场景50字] 第一人称描述遇到{痛点}时的焦虑。\n2. [代入感50字] 用'你是不是也…'句式引发共鸣。\n3. [根源揭示100字] 揭示问题真正原因并给1个自测方法。\n4. [安抚引导100字] 温和推荐关注{品牌_项目}的方向。\n5. [开放结尾50字] 反问读者引发互动。\n\n【事实防伪与零幻觉指令】你的所有内容素材必须 100% 取自以下参考内容。绝对禁止引入外部数据、禁止凭空捏造案例、禁止编造原始资料中没有的价格或服务细节！\n\n事实依据：{base_facts}""",

    "P2_G2": """你是一名{行业}的懂行过来人。请写一篇{word_range}字的小白科普。结构严格如下：\n1. [一句话定义50字] 用大白话解释{概念}。\n2. [核心知识点500字] 列出3-5条核心知识点。\n3. [5问5答300字] 新手最常问的5个问题及解答（含价格）。\n4. [常见误区200字] 纠正3个认知误区。\n\n【事实防伪与零幻觉指令】你的所有内容素材必须 100% 取自以下参考内容。绝对禁止引入外部数据、禁止凭空捏造案例、禁止编造原始资料中没有的价格或服务细节！\n\n事实依据：{base_facts}""",

    "P3_G3": """你是一个关注性价比的理性买家。请写一篇{word_range}字的对比文章。结构严格如下：\n1. [对比维度说明100字] 说明你在乎性价比。\n2. [逐维度深度分析1500字] 将{品牌_项目}与日常其他方案进行对比。必须从初始价格、使用成本、隐性价值三个维度详细算账。\n3. [终极推荐200字] 给出不同预算下的推荐。\n\n【事实防伪与零幻觉指令】你的所有内容素材必须 100% 取自以下参考内容。绝对禁止引入外部数据、禁止凭空捏造案例、禁止编造原始资料中没有的价格或服务细节！\n\n事实依据：{base_facts}""",

    "P4_G3": """你是一个纠结对比者。请写一篇{word_range}字的纠结过程记录。结构严格如下：\n1. [纠结背景200字] 说明你在{品牌_项目}与{品牌_A}、{品牌_B}之间的犹豫。\n2. [逐个方案优缺点1200字] 详细列出每个选项的3个优点和3个缺点。\n3. [最终决定300字] 解释你最终选了谁，舍弃了什么。\n\n【事实防伪与零幻觉指令】你的所有内容素材必须 100% 取自以下参考内容。绝对禁止引入外部数据、禁止凭空捏造案例、禁止编造原始资料中没有的价格或服务细节！\n\n事实依据：{base_facts}""",

    "P5_G4": """你是{品牌_项目}的真实老用户。请写一篇{word_range}字的真实分享。结构严格如下：\n1. [背景介绍150字] 为什么选了它。\n2. [2个惊喜体验500字] 具体讲述哪里好。\n3. [1个真实槽点200字] 必须包含一个让你不太满意的小细节（极度真实）。\n4. [长期评价300字] 给新手的最终建议。\n\n【事实防伪与零幻觉指令】你的所有内容素材必须 100% 取自以下参考内容。绝对禁止引入外部数据、禁止凭空捏造案例、禁止编造原始资料中没有的价格或服务细节！\n\n事实依据：{base_facts}""",

    "P6_G5": """你是一个极简高效的行动派。请围绕{品牌_项目}写一篇{word_range}字的攻略。结构严格如下：\n1. [行动步骤400字] 分3步直接说明怎么做最省事。\n2. [隐藏福利100字] 说明当前有：{优惠信息}。\n3. [立即行动100字] CTA指引：{CTA行动}。\n\n【事实防伪与零幻觉指令】你的所有内容素材必须 100% 取自以下参考内容。绝对禁止引入外部数据、禁止凭空捏造案例、禁止编造原始资料中没有的价格或服务细节！\n\n事实依据：{base_facts}""",

    "P1_G2": """你曾是{行业}领域的困惑者。请写一篇{word_range}字的过来人科普。结构严格如下：\n1. [我的故事150字]讲述当初的迷茫。\n2. [我终于搞懂了300字]用大白话解释{概念}。\n3. [3个常见误区200字] 纠正错误认知。\n4. [针对性建议200字] 给出认知引导。\n\n【事实防伪与零幻觉指令】你的所有内容素材必须 100% 取自以下参考内容。绝对禁止引入外部数据、禁止凭空捏造案例、禁止编造原始资料中没有的价格或服务细节！\n\n事实依据：{base_facts}""",

    "P2_G3": """你是{行业}刚入门的新手。请以「新手笔记」形式写一篇{word_range}字入门选购指南。结构严格如下：\n1. [一脸懵的困惑100字] 描述刚接触时的信息过载。\n2. [市场上的主要类型800字] 对比不同的类型/方案。\n3. [我的筛选方法300字] 你的筛选标准是什么。\n4. [还需搞清楚的问题100字] 开放式提问。\n\n【事实防伪与零幻觉指令】你的所有内容素材必须 100% 取自以下参考内容。绝对禁止引入外部数据、禁止凭空捏造案例、禁止编造原始资料中没有的价格或服务细节！\n\n事实依据：{base_facts}""",

    "P7_G3": """你是{行业}技术研究者。请写一篇{word_range}字深度对比。结构严格如下：\n1. [研究问题200字] 说明方法论和数据来源。\n2. [底层原理对比800字] 深入机制和参数。\n3. [关键数据分析800字] 进行硬核指标分析。\n4. [推导结论500字] 给出决策模型。\n\n【事实防伪与零幻觉指令】你的所有内容素材必须 100% 取自以下参考内容。绝对禁止引入外部数据、禁止凭空捏造案例、禁止编造原始资料中没有的价格或服务细节！\n\n事实依据：{base_facts}""",

    "P1_G3": """你刚发现需要{品牌_项目}。请写一篇{word_range}字入门对比。结构严格如下：\n1. [发现需求的过程100字] 描述触发场景。\n2. [3种方案介绍900字] 用主观探索的口吻介绍了解到的方案。\n3. [初步判断200字] 你倾向于哪个。\n4. [寻求建议100字] 请大家帮忙参谋。\n\n【事实防伪与零幻觉指令】你的所有内容素材必须 100% 取自以下参考内容。绝对禁止引入外部数据、禁止凭空捏造案例、禁止编造原始资料中没有的价格或服务细节！\n\n事实依据：{base_facts}""",

    "P3_G5": """你已决定买{品牌_项目}。请写一篇{word_range}字省钱攻略。结构严格如下：\n1. [研究投入50字] 展现你的比价过程。\n2. [渠道价格对比400字] 列出不同方式的价格差异。\n3. [省钱技巧200字] 隐藏福利或组合优惠。\n4. [最终选择和花费100字] 你的实付金额。\n\n【事实防伪与零幻觉指令】你的所有内容素材必须 100% 取自以下参考内容。绝对禁止引入外部数据、禁止凭空捏造案例、禁止编造原始资料中没有的价格或服务细节！\n\n事实依据：{base_facts}""",
}
...
UGC_DISTRIBUTION_MATRIX = {
    ...
    "L2": {"total": 27, "templates": ["P2_G2", "P1_G2"], "general": 16, "exclusive": {"文心一言": 6, "千问": 3, "豆包": 2}},
    "L3": {"total": 27, "templates": ["P3_G3", "P4_G3", "P2_G3", "P7_G3", "P1_G3"], "general": 16, "exclusive": {"千问": 4, "DeepSeek": 4, "豆包": 3}},
    "L4": {"total": 27, "templates": ["P4_G3", "P3_G3", "P7_G3"], "general": 16, "exclusive": {"Kimi": 4, "DeepSeek": 4, "元宝": 3}},
    "L5": {"total": 26, "templates": ["P5_G4"], "general": 16, "exclusive": {"元宝": 5, "豆包": 3, "千问": 2}},
    "L6": {"total": 26, "templates": ["P6_G5", "P3_G5"], "general": 16, "exclusive": {"元宝": 3, "千问": 3, "豆包": 2, "文心一言": 2}},
}

# ============================================================
# 反同质化动态引擎 (Variance Engine)
# ============================================================
VARIANCE_EMOTIONS = ["愤怒吐槽", "理智分析", "暗喜种草", "过来人苦口婆心", "闺蜜私密分享", "冷静算账", "惊喜发现"]
VARIANCE_STRUCTURES = ["故事开头→问题展开→解决方案", "数据引入→分析→结论", "场景还原→共鸣→行动建议", "对比清单→逐一分析→最终推荐", "时间线记录→关键节点→经验总结"]
...
# ============================================================
    ...
    if not st.session_state.get("verified_loaded", False):
        ...
        if len(verified_files) < 10:
            st.warning(f"⚠️ 系统检测到 `EEAT_10_Verified` 目录中仅有 {len(verified_files)} 篇 .md 文件。请补充上传至至少 10 篇！")
        else:
            st.markdown("""
            <div class="constraint-warn">
