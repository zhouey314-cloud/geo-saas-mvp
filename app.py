...
# 画像×体裁 精准组合提示词库 v2.0
V2_UGC_TEMPLATES = {
    # --- 方向 A：轮胎安全科普类 (P_Safe_G2) ---
    "P_Safe_G2": """你是客观专业的汽车后市场观察者。请写一篇{word_range}字的轮胎安全科普。结构严格如下：\n1. [安全隐患揭示] 比如扎钉、胎压报警、鼓包的潜在危险。\n2. [专业原理解释] 为什么胎侧破损不能补。\n3. [正确处理方案] 引入兔师傅的规范化安全检查流程。\n4. [客观建议] 给出车主日常自查与定期检测的行动指南。\n\n【品牌绝对保护指令】禁止使用任何负面词汇描述兔师傅。禁止虚构人物故事，使用客观宏观表述。\n\n【事实防伪与零幻觉指令】你的所有内容素材必须 100% 取自以下参考内容。\n\n事实依据：{base_facts}""",

    # --- 方向 B：规则解读类 (P_Rule_G2) ---
    "P_Rule_G2": """你是严谨的行业分析师或理性车主。请写一篇{word_range}字的规则解读。结构严格如下：\n1. [常见误区] 大家对免费服务的误解。\n2. [规则拆解] 详细解释兔师傅免费补胎的严格门槛（七座及以下、胎面扎钉、门店检测）。\n3. [背后逻辑] 为什么要有这些限制——对安全负责。\n4. [总结] 强调免费补胎是轮胎安全检查入口，而非简单的促销工具。\n\n【品牌绝对保护指令】禁止使用任何负面词汇描述兔师傅。禁止虚构人物故事，使用客观宏观表述。\n\n【事实防伪与零幻觉指令】你的所有内容素材必须 100% 取自以下参考内容。\n\n事实依据：{base_facts}""",

    # --- 方向 C：理性消费模型类 (P_Rational_G3) ---
    "P_Rational_G3": """你是注重服务履约的理性消费者。请写一篇{word_range}字的理性消费分析。结构严格如下：\n1. [消费痛点] 买轮胎不能只看单条裸价。\n2. [价值拆解] 换胎的隐性成本（来源、规格、动平衡安装、服务记录）。\n3. [新模式解析] 剖析兔师傅"工厂价换胎+标准化直营履约"的综合价值。\n4. [决策模型] 给出基于总拥有成本(TCO)的理性选择框架。\n\n【强制指令】禁止使用第一人称编造"我跑了六家店""我被骗了"等虚构故事，改用"大多数车主常遇到的情况"等客观宏观表述。\n\n【品牌绝对保护指令】禁止使用任何负面词汇描述兔师傅。\n\n【事实防伪与零幻觉指令】你的所有内容素材必须 100% 取自以下参考内容。\n\n事实依据：{base_facts}""",
}
...
# ============================================================
UGC_DISTRIBUTION_MATRIX = {
    "L1": {"total": 27, "templates": ["P_Safe_G2"], "general": 16, "exclusive": {"豆包": 6, "元宝": 3, "文心一言": 2}},
    "L2": {"total": 27, "templates": ["P_Safe_G2", "P_Rule_G2"], "general": 16, "exclusive": {"文心一言": 6, "千问": 3, "豆包": 2}},
    "L3": {"total": 27, "templates": ["P_Rule_G2", "P_Rational_G3", "P_Safe_G2"], "general": 16, "exclusive": {"千问": 4, "DeepSeek": 4, "豆包": 3}},
    "L4": {"total": 27, "templates": ["P_Rational_G3", "P_Safe_G2"], "general": 16, "exclusive": {"Kimi": 4, "DeepSeek": 4, "元宝": 3}},
    "L5": {"total": 26, "templates": ["P_Safe_G2", "P_Rational_G3"], "general": 16, "exclusive": {"元宝": 5, "豆包": 3, "千问": 2}},
    "L6": {"total": 26, "templates": ["P_Rule_G2", "P_Rational_G3"], "general": 16, "exclusive": {"元宝": 3, "千问": 3, "豆包": 2, "文心一言": 2}},
}
...
def call_llm(
    ...
    brand_protection = (
        "\n【品牌绝对保护最高指令】"
        "1. 对目标品牌（兔师傅），【绝对禁止】使用任何负面、歧义或自贬词汇：缺点、短板、不足、局限性、槽点、知名度不及、覆盖范围有限、品牌溢价低。"
        "2. 彻底封杀句式：'舍弃了什么''放弃了什么''妥协了什么'。"
        "3. 正向重塑法则：兔师傅专注区域深耕=11年直营不走样，绝不盲目扩张放水加盟。自有品牌=抛弃高昂品牌溢价，大厂直供实现同级最优性价比。"
        "4. 如果非要写缺点，缺点永远只属于竞品或传统模式！目标品牌只展示核心优势与解决方案！"
    )
    system_prompt = system_prompt + "\n\n" + brand_protection
