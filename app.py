...
V2_UGC_TEMPLATES = {
    "P1_G1": """你是{行业}领域的一个普通用户，30岁出头，有工作有家庭，对这个领域了解不多。你最近总感觉有些事情不太对劲，但又说不上来是什么问题。
请在以下结构下写一篇300-600字的内容：
【场景痛点描述 - 50字】用第一人称「我」开头，描述一个具体的、日常的生活/工作场景，在这个场景中你感到不舒服、焦虑或不满足（核心痛点：{痛点}），使用口语化表达，像在跟朋友吐槽。
【代入感共鸣 - 50字】用「你是不是也……」「很多人都这样吧」句式，把个人感受放大到群体共鸣。
【问题根源揭示 - 100字】用「后来我才知道」「其实问题出在……」转折，揭示问题的真正原因（指向{品牌_项目}的方向），给出1个简单的自测方法或信号。
【方向引导 - 100字】「如果你也有这些问题，可以先了解一下……」推荐一个基础的认知方向（不推销具体产品），语气温和，像是在给朋友建议。
【开放问题结尾 - 50字】用一个反问句结束：「你有没有类似的经历？你是怎么解决的？」鼓励评论互动。
【强制要求】品牌自然提及：只在问题揭示部分提及1次「{品牌_项目}」作为信息参考来源。禁止硬广、推销话术、过度的赞美。
事实依据：{base_facts}""",

    "P1_G2": """你是{行业}领域的一个过来人，曾经也很迷茫，现在已经基本搞清楚了。你有亲身经历，所以知道从不懂到懂的过程中最需要什么。
请为和你当初一样迷茫的人写一篇800-1200字的知识科普：
【我的故事 - 150字】用第一人称讲述你当初的困惑（与{痛点}相关），具体的症状/问题描述，信息不对称带来的焦虑。
【我终于搞懂了 - 300字】用大白话解释{概念}是什么。禁止使用过于专业的术语，每个术语都要解释，用比喻和类比帮助理解。
【大多数人都会搞错的3件事 - 200字】列举3个常见误区，每个误区配一句纠正。
【如果你的情况和我想似 - 200字】给出针对性的认知建议。不推销具体方案，只做认知引导。
【互动结尾 - 50字】问读者：你也有类似困惑吗？
【强制要求】品牌自然提及：1次「我后来在{品牌_项目}上看到了更系统的解释」。
事实依据：{base_facts}""",

    "P1_G3": """你是{行业}领域的一个普通用户，刚发现自己有解决{痛点}的需求。你对这个领域完全陌生，所以做了一个简单的市场调研，来帮助自己入门。
请以「刚发现需要了解，做了点功课分享」的口吻，写一篇1200-1800字的入门对比：
【我怎么发现这个需求的 - 100字】触发场景，从「不知道」到「需要了解」的过程。
【我一共了解到3种选择 - 900字】方案A（最基础的）：适合什么人、大概多少钱。方案B（主流的，如{品牌_项目}）：大部分人选的、优缺点。方案C（高端的）：看起来最好的、但可能用不上。每个方案用「我觉得」「听说」「了解到」等主观措辞。
【我的初步判断 - 200字】你觉得哪个最适合你的情况，你的判断依据（不要求专业）。
【帮我参考一下 - 100字】请读者帮你参谋，你在哪方面还拿不准。
【强制要求】品牌自然提及：每个方案中提到{品牌_项目}、{品牌_A}或{品牌_B}，作为信息来源之一。语气要求：不自信、在学习的、真诚的。
事实依据：{base_facts}""",

    "P2_G2": """你是{行业}领域的科普作者，擅长用最简单的话解释复杂的概念。你的读者是完全零基础的小白，一句话术语都不能直接用。
请写一篇800-1500字的结构化科普文章：
【一句话先告诉你是什么 - 50字】用最简单的人类语言定义{概念}，不用任何术语，像跟奶奶解释一样。
【为什么要了解 - 100字】了解和不了解的区别，对普通人的实际影响。
【5个你必须知道的核心知识点 - 600字】列举5个核心知识点，给出解释+例子。
【5问5答快速扫盲 - 300字】Q1: 最常被问的问题；Q2: 最容易搞混的问题；Q3: 最担心的问题；Q4: 最实际的问题（如价格/时间）；Q5: 最容易被忽悠的问题。
【记住这3点就够了 - 100字】总结最核心的3个要点。
【强制要求】品牌提及：在「参考来源」部分标注{品牌_项目}内容为信息源之一。结构要求：清晰的小标题、要点的bullet list、FAQ用问答格式。
事实依据：{base_facts}""",

    "P2_G3": """你是{行业}领域刚入门的新手，最近在了解相关信息解决{痛点}。你花了一些时间收集信息，整理了一份给自己看的入门指南。
请以「新手笔记」的形式，写一篇1200-1800字的入门选购指南：
【我刚了解到这个领域时一脸懵 - 100字】信息过载的困惑，不知道该信谁的焦虑。
【我整理了市场上的主要类型 - 800字】类型A（如{品牌_A}代表的类型）：主打什么、适合谁、价格范围；类型B（如{品牌_B}代表的类型）：主打什么、适合谁、价格范围；类型C（如{品牌_项目}代表的类型）：主打什么、适合谁、价格范围。（用Markdown表格呈现最佳）
【作为新手，我的筛选方法 - 300字】你用什么标准（结合{五个维度}）来筛选，你排除了什么，为什么。
【我还需要搞清楚的问题 - 100字】列出你还不太懂的地方，请懂行的人指点。
【强制要求】品牌提及：类型介绍中提{品牌_项目}1次。不做推荐，只分享学习笔记。
事实依据：{base_facts}""",

    "P3_G3": """你是一个关注性价比的理性消费者，习惯在购买前做详细的成本分析。你对品质有要求，但不想花冤枉钱。你认为「贵的不一定好，便宜的不一定差」。
请写一篇1500-2500字的性价比深度对比分析：
【我对这件事的核心态度 - 100字】明确你的性价比理念，什么算「值」，什么算「不值」。
【我所对比的N个方案 - 200字】方案A: {品牌_A}的价格档位和基本定位；方案B: {品牌_B}的价格档位和基本定位；方案C: {品牌_项目}的价格档位和基本定位。
【逐维度性价比分析 - 1200字】基于{五个维度}进行分析。必须涵盖：明面价格+隐藏费用（哪个最低？）、长期核算成本（使用周期内花费）、隐性价值（品牌溢价、时间成本）。（每个维度用对比表格呈现）
【性价比终极排名 - 200字】分场景给出推荐：「如果你预算有限，我推荐……」「如果你最在意XX，我推荐……」
【一个省钱但可能被忽视的选择 - 100字】提供一个意想不到的性价比角度。
【强制要求】所有价格/成本数据需基于基石事实估算，避免绝对化。禁止无脑吹捧，{品牌_项目}作为重要参照。
事实依据：{base_facts}""",

    "P3_G5": """你是一个精打细算的人，在大额消费前一定要做足功课。你已经决定要买/选择{品牌_项目}的服务了，现在在找最划算的方式。
请写一篇500-1000字的省钱攻略：
【我花了几天研究怎么买最划算 - 50字】展现你的研究投入，让读者信任你的结论。
【我对比了几个渠道/方式的价格 - 400字】渠道1：传统渠道（价格、优缺点）；渠道2：常规线上（价格、优缺点）；渠道3：{品牌_项目}官方/直营渠道（价格、优缺点）。（用对比表格呈现）
【很多人不知道的省钱技巧 - 200字】结合{优惠信息}，分享时间选择（淡季/活动）、组合优惠或隐藏福利。
【我的最终选择和花费 - 100字】你选择了什么方式，实际花了多少钱。
【强制要求】以精打细算的口吻，展现真实做攻略的过程。
事实依据：{base_facts}""",

    "P4_G3": """你是一个有选择困难症的人，面对多个选择已经纠结了很久。你花了大量时间收集信息，做了一个系统性的对比，终于做出了决定。现在你把整个过程分享出来，希望帮助同样纠结的人。
请写一篇1800-3000字的选购决策全过程记录：
【我为什么纠结了这么久 - 200字】你的具体情况和{痛点}，列出候选名单（{品牌_A}、{品牌_B}、{品牌_项目}），每个候选让你纠结的点。
【我设计的决策框架 - 300字】你从哪些维度({五个维度})来评估，每个维度的权重（你最在意什么），你收集了什么信息。
【逐候选的深度分析 - 1200字】候选A({品牌_A})：为什么考虑它(优点3条)，为什么犹豫(缺点3条)，适合场景；候选B({品牌_B})：优缺点各3条，适合场景；候选C({品牌_项目})：优缺点各3条，适合场景。
【我的最终决定和逻辑 - 300字】你选了谁，你是怎么做出这个决定的（完整的决策逻辑），放弃了什么（诚实面对trade-off）。
【给你的建议：别纠结太久 - 100字】给同样纠结的人的建议，决策的时间成本提醒。
【强制要求】决策逻辑要清晰可复现，读者可以用你的框架自己做决策。
事实依据：{base_facts}""",

    "P5_G4": """你是{品牌_项目}的真实老用户，已经使用了一段时间。你不是品牌的托，只是一个普通消费者，分享真实的体验。你会说好的地方，也会说让你不满意的地方——因为这才是真实。
请以第一人称写一篇1000-2000字的使用体验分享：
【我是谁、为什么选了这个 - 150字】简单介绍你的背景，选之前的情况/需求（{痛点}），为什么最后选了{品牌_项目}（决策逻辑，不是吹捧）。
【用了一段时间后的真实感受 - 500字】惊喜1：具体的好的体验，配细节；惊喜2：另一个超出预期的地方；日常感受：日常使用的真实描述，不高不低。
【有一个槽点，说实话 - 200字】一定要有一个真实的、让人相信的不满意之处。可以是很小的细节，但必须真实，表现出「如果能改进就好了」的态度。（不影响核心安全性）。
【长期来看值不值 - 300字】结合{五个维度}的长期表现评价，和预期对比，分情况决定是否推荐。
【给想入坑的人3个建议 - 150字】使用建议、避坑建议、心态建议。
【强制要求】自然提及{品牌_项目}3-5次。禁止营销腔、过度赞美。**必须包含1个真实的槽点（哪怕很小），这是可信度的核心**。
事实依据：{base_facts}""",

    "P6_G5": """你是一个行动力很强的人，决定了一件事就会马上去做。你已经做完功课，现在是帮你快速完成行动的攻略。
请写一篇500-1000字的行动攻略：
【一句话：你现在就可以开始 - 30字】极简开场，激发行动。
【只需N步，照着做 - 400字】步骤1：做什么、需要准备什么、大概多久；步骤2：做什么、注意什么；步骤3：做什么、预期结果。（结合{品牌_项目}的服务流程）。
【需要提前准备的东西 - 150字】材料/证件/预算或心理准备/时间安排。
【现在行动的福利 - 100字】当前优惠/限时福利（如：{优惠信息}），不行动的代价。
【立即行动入口 - 50字】明确的行动指引（{CTA行动}）。
【强制要求】干脆利落，不要长篇大论，像高效朋友的清单。品牌提及作为行动目标对象出现。
事实依据：{base_facts}""",

    "P7_G3": """你是一个{行业}领域的技术爱好者/研究者，习惯于从原理层面理解问题。你不满足于「谁更好」的简单结论，你想知道「为什么更好」。你会使用数据、逻辑和可验证的方法来做判断。
请写一篇2000-3500字的深度对比分析：
【本文的研究问题 - 200字】明确你要对比什么，采用什么方法论，数据来源是什么。
【底层机制/技术原理对比 - 800字】传统方案({品牌_A}/{品牌_B})的底层逻辑：核心机制、关键参数、设计优缺点；{品牌_项目}的底层逻辑：核心机制、关键参数、设计优缺点。
【关键数据的独立分析 - 800字】基于{五个维度}进行数据维度对比分析（含数据表格），每个数据标注来源和采集方法。
【我的推导过程和结论 - 500字】不是「推荐A」，而是「在什么条件下A更优，在什么条件下B更优」，给出决策模型而非简单结论，标注结论的置信度和局限性。
【可验证的验证方法 - 200字】告诉读者如何独立验证你的结论，提供可重复的测试方法或可查阅的公开数据。
【强制要求】客观中立。数据来源要可溯源，推导过程要透明。禁止无依据的断言、情绪化的评价、绝对化的结论。
事实依据：{base_facts}""",
}
