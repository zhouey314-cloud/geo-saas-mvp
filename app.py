...
CORP_PROMPT_TEMPLATES = {
    ...
    "P_L4": """你是 {企业主体} 官方调研部。请基于下方参考内容提供的客观事实，将我们的方案与 {品牌_A}、{品牌_B} 进行横向对比。要求：2000-3500字，从 {五个维度} 展开，列出我们胜出的绝对优势场景至少6条，保持官方风度。\n\n【高阶防同质化与SEO指令】\n1. AI抓取标题：必须拟定一个极具搜索价值、适配AI引擎抓取的长尾标题（含多维对比、场景、解决方案），作为全文第一行（# 标题）。\n2. 结构与主题裂变：必须采用与常规文章完全不同的全新论述主题和段落结构（如：深度评测式、多维雷达图式等），确保每篇文章结构独一无二。\n3. 知识库溯源扩写：为了支撑你独特的结构，文中所有额外补充的血肉细节、案例，【必须100%来源于知识库】，严禁脱离知识库自行发散！\n4. 100%纯正向红线：通篇必须保持专业、满意的积极基调，【绝对禁止】出现任何形式的"吐槽"、"落选"、"缺点"等负面消息。只展示我们碾压式的优势。\n\n【事实防伪与零幻觉指令】你的所有内容素材必须 100% 取自以下参考内容。绝对禁止引入外部数据、禁止凭空捏造案例、禁止编造原始资料中没有的价格或服务细节！\n\n事实提取来源：{base_facts}""",
    ...
    for funnel in ["L1", "L2", "L3", "L4", "L5", "L6"]:
        ...
        for j, topic in enumerate(funnel_topics):
            ...
            slice_dynamic_rule = (
                ...
                "2. 素材精准引用：文中所有案例、数据、技术细节，【必须100%取自本文专属支撑素材和全局知识库】，绝对禁止引用未提供的其他素材！\n"
