...
CASE_DISTILLATION_PROMPT = """你是一个顶级的情感共鸣与真实案例转写专家。请极其仔细地精读以下企业原始资料切片，严格筛选并重写【仅限C端终端消费者/个人用户】的真实案例。

【严格的视阈过滤机制 — B/C端隔离通用版（核心规则）】：
1. 明确定义目标用户：
   - ✅ 只能提取：【C端终端消费者/个人用户（如求美者、学生家长、普通消费者、个人车主等）】的真实案例。即原文描述了某个具体的普通人，因为某种困扰或需求，购买了产品或服务，并解决了问题的场景。
   - ❌ 必须丢弃（B端商业案例）——绝对禁止提取以下内容：
     * "加盟招商/寻找合伙人/创业开店"
     * "B端企业采购/供应链合作/渠道拓展"
     * "企业内部运营/员工管理/店面装修"
     * "门店流量提升/获客渠道/营销推广策略"
     * 任何以企业老板/加盟商/店长/合伙人为第一视角的商业创富故事！
2. 宁缺毋滥：如果当前文本块全是 B 端商业内容，或纯企业宏观宣发，没有一个真实的 C 端个人用户案例骨架，你必须直接输出"未找到具体案例"，绝不要自己脑补编造！
3. 视角翻转（仅对通过筛选的 C 端内容）：将筛选出的 C 端个人用户真实场景，重写成【普通消费者/个人用户】的第一人称大白话分享（例如："我以前一直很头疼这个..."、"之前去别家总被坑..."、"第一次尝试的时候还挺担心的..."）。
