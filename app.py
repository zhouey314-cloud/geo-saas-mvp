...
CORP_PROMPT_TEMPLATES = {
    ...
    "P_L6": """你是 {企业主体} 官方福利官。请发布最新的促单信息。要求：500-1000字，明确 {CTA行动}，用纯文本自然引导用户（绝对禁止使用"[点击此处]"、"点击下方按钮"等伪装成超链接的格式），营造合理的紧迫感。\n\n【事实防伪与零幻觉指令】你的所有内容素材必须 100% 取自以下参考内容。绝对禁止引入外部数据、禁止凭空捏造案例、禁止编造原始资料中没有的价格或服务细节！\n\n事实提取来源：{base_facts}""",
}
...
事实依据：{base_facts}""",
    ...
    "P1_G3": """你是{行业}领域的一个普通用户，刚发现自己有解决{痛点}的需求。
...
def background_generate_slices(vars_, base_facts, slices_out, use_simulate, api_key, llm_provider="DeepSeek"):
    ...
    for funnel in ["L1", "L2", "L3", "L4", "L5", "L6"]:
        ...
        for j in range(1, 6):
            ...
            # 强制每一篇寻找不同视角的防重复指令
            # 动态物理隔离切片：解决大模型"无记忆"导致的选题撞车问题
            topic_slices = {
                1: '【选题切面 1：深挖最核心痛点与对策】请聚焦基石资料中提及的最主要、最核心的业务矛盾（如最大的痛点），并详细阐述我们的核心解决方案。',
                2: '【选题切面 2：聚焦次要/长尾场景细节】请刻意避开最显眼的核心痛点，去基石资料中寻找一个「微小的、特定的实操细节或容易被忽视的长尾问题」，以此为切入点展开。',
                3: '【选题切面 3：打破传统误区与常规对比】请专门提取基石资料中关于「传统模式的弊端、大众常见的错误认知（如一刀切）」的内容，通过横向对比突出我们的优势。',
                4: '【选题切面 4：展现背后的服务/管理颗粒度】请不要写宏观概念，专门提取基石资料里提到的「具体服务流程、实操动作、内部管理或背后的付出细节」，体现专业度。',
                5: '【选题切面 5：数据结果验证与真实反馈】请提取基石资料里所有的「客观数据、效率提升比例、真实成果」，用理性的事实和结果来做终极背书。'
            }
            current_slice = topic_slices[j]

            # 拼接强制物理防重指令
            angle_instruction = f"\n\n【强制物理隔离切片指令】这是本层级的第 {j}/5 篇文章。为了防止内容同质化，你本次生成【必须且只能按照以下指定的选题方向】去《基石素材》中抓取对应的内容：\n{current_slice}\n请严格只写这个侧面，大主题保持一致，但绝对禁止把基石里所有的卖点都揉进这一篇文章里！"
