...
CASE_DISTILLATION_PROMPT = """你是一个顶级的情感共鸣与真实案例转写专家。请极其仔细地精读以下企业原始资料切片，严格筛选并重写真实客户案例。

【严格的视阈过滤机制（核心规则）】：
1. 宁缺毋滥，严禁"无中生有"：你必须极其严格地区分两类内容！
   - ❌ 必须丢弃：纯企业宏观宣发（如"我们拥有XX年历史"、"始终秉持XX理念"）、技术参数罗列（如"采用XX材料，耐温XX度"）、企业自夸（如"行业领先"、"遥遥领先"）。
   - ✅ 可以重写：只有原文中实实在在地描述了【客户/用户的具体使用场景、痛点遭遇、解决方案】的段落，你才可以提取出来进行重写。
2. 判定标准：如果当前文本块完全是企业官方视角的自我介绍或广告，连一个具体的客户案例骨架都没有，你必须直接输出"未找到具体案例"，绝不要自己脑补编造！
3. 视角翻转（仅对通过筛选的内容）：将上述通过筛选的真实场景，重写成【符合该资料所属行业的真实客户/消费者】的第一人称大白话分享（例如："我以前一直很头疼这个..."、"之前去别家总被坑..."）。
4. 保留核心事实：重写时，必须 100% 无损保留原文中的客观数据、痛点细节和最终效果，绝不能编造原文没有的参数。
5. 格式强制：每个重写后的案例之间【必须】用三个减号（---）分隔，使用真实的回车换行。如果没有发现任何可用的客户场景，只输出"未找到具体案例"。
