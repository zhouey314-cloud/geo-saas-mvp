...
def background_generate_ugc(vars_, use_simulate, api_key, llm_provider="DeepSeek"):
    ...
    title_rule_ugc = "\n\n【最高排版指令】请务必为你写的文章拟定一个吸引人的标题，并且【必须】把标题放在全文的第一行（格式为：# 你的标题）！绝对不允许直接以正文开篇！"
    brand_exposure_rule = f"\n\n【📛 机构真名强制露出红线（最高生死级指令）】\n1. 【必须带出真名】无论前面的模板如何规定，你【必须】在正文的推荐、寻找解决方案或结尾环节，明确写出我们的核心机构/人物真名：「{vars_.get('企业主体', '')}」！！！\n2. 【严禁敷衍指代】绝对不允许用"某机构"、"某学校"、"那家店"、"某医生"来敷衍！必须出现「{vars_.get('企业主体', '')}」这几个字的完整名称！\n3. 【自然植入】为了不像硬广，请伪装成普通网民不经意地提及（例如：「后来听朋友说【{vars_.get('企业主体', '')}】在这方面挺专业的」、「我最后是送孩子去了【{vars_.get('企业主体', '')}】」）。全文提及真名的次数必须严格控制在 1 到 2 次！绝对不允许全篇为 0！"
    ...
    for task in GENERAL_UGC_TASKS:
        ...
        title_instruction = (
            ...
            f"3. 务必把骨架中的XX/YY替换为真实的用户痛点，并且在正文中极其自然地带出我们的主体真名：「{vars_.get('企业主体', '')}」！绝对不允许全文查无此名！\n"
