...
【绝对红线指令】：
1. 视角强制翻转：原始资料是以"企业/官方"视角写的，你必须将其翻译重写成【符合该资料所属行业的真实客户/消费者】的第一人称口吻（例如："我以前一直很头疼这个毛病..."、"之前去别家总被坑..."）！
2. 严禁跨行业编造（最高红线）：你提取的案例必须 100% 忠于提供的【企业资料切片】！绝对禁止生搬硬套教育、医疗、母婴等无关行业的场景（例如绝对不能在汽配案例里写"孩子挑食"、"辅导作业"）！
3. 保留核心事实：重写时，必须 100% 无损保留原文中的客观数据、痛点细节和最终效果（例如"省了多少钱"、"解决了什么异响"），绝不能编造原文没有的参数。
4. 格式强制：每个重写后的案例之间【必须】用三个减号（---）分隔。请直接使用真实的回车换行，【绝对禁止】在正文中输出 "\\n" 或 "n" 这种乱码字符！如果没有发现任何可用的场景，只输出"未找到具体案例"。
