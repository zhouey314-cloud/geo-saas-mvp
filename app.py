...
def background_generate_slices(vars_, base_facts, slices_out, use_simulate, api_key, llm_provider="DeepSeek"):
    ...
    for funnel in ["L1", "L2", "L3", "L4", "L5", "L6"]:
        ...
        for j in range(1, 6):
            ...
            # 强制每一篇寻找不同视角的防重复指令
            # 动态抽题分配器：强制大模型基于10篇基石素材，在后台拆分出5个独立话题，并认领第j个。
            angle_instruction = f"""
\n\n【基石素材动态拆题与去同质化绝对红线】
这是同层级 5 篇文章中的第 {j} 篇！为了彻底贯彻"从 10 篇基石文章中提炼不同选题"的要求，请你严格执行以下纪律：
1. 【后台素材拆题】请全面扫描下方的《基石素材》全文，根据本层级的任务，从中梳理出 5 个截然不同、互不重叠的【微观细分选题】（例如：基石中提到的5个不同的痛点场景、5个不同的坏习惯、或5个不同的解决细节）。
2. 【严格认领第 {j} 题】你本次【必须且只能】聚焦于你梳理出的这 5 个细分选题中的**「第 {j} 个」**进行独立创作！绝对禁止把基石里的所有卖点都大杂燴式地揉进这一篇文章里。
3. 【标题去宏观化】文章的标题必须极其锐利地聚焦在你选定的这【第 {j} 个】微观细分点上（比如针对某个具体的坏习惯）。【绝对禁止】在标题里像复读机一样生硬套用宏大概念（如：严禁在标题中直接使用「{vars_.get('痛点', '')}」或「{vars_.get('概念', '')}」的字眼）！如果这 5 篇文章的标题看起来雷同，将被判定为严重生产事故！
"""
